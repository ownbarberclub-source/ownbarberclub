...
import avenidaBruno from "./assets/avenida-bruno.jpg";
// Equipe Efapi
    ...
    <div className="min-h-screen bg-black text-white selection:bg-brand selection:text-black font-sans">
      ...
      <div className="fixed bottom-8 right-8 z-[60] flex flex-col gap-4 items-end">
        ...
        <motion.button
          ...
          className="w-14 h-14 bg-brand rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-shadow border-2 border-white/20"
        >
        ...
        {isBookingOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg bg-charcoal border-4 border-white/10 p-8 md:p-12 shadow-[30px_30px_0px_0px_rgba(225,6,0,0.1)]"
            >
              <button 
                onClick={() => setIsBookingOpen(false)}
              ...
              <div className="mb-10 text-center">
                <h3 className="text-4xl md:text-5xl mb-2">ESCOLHA A UNIDADE.</h3>
                <p className="text-brand font-mono text-xs uppercase tracking-widest">Onde você quer dar o trato?</p>
              </div>

              <div className="flex flex-col gap-4">
                {[
                  { name: "Centro", link: "https://sites.appbarber.com.br/link/14836635/ownbarberclubcentro", addr: "Av. Fernando Machado" },
                  { name: "Efapi", link: "https://sites.appbarber.com.br/link/18873717/ownbarberclubefapi-8pu2", addr: "R. Maravilha" },
                  { name: "Avenida", link: "https://sites.appbarber.com.br/link/20469036/ownbarberclubav-h2tx", addr: "Av. Getúlio Vargas" },
                ].map((unit) => (
                  <a
                    key={unit.name}
                    href={unit.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group flex items-center justify-between p-6 border-2 border-white/10 hover:border-brand bg-black/50 transition-all hover:translate-x-2"
                  >
                    <div>
                      <h4 className="text-2xl italic mb-1 group-hover:text-brand transition-colors">{unit.name}</h4>
                      <p className="text-xs text-white/50 font-mono uppercase">{unit.addr}</p>
                    </div>
              ...
              <p className="mt-10 text-center text-[10px] text-white/30 uppercase tracking-widest font-mono">
                Agendamento via AppBarber
              </p>
            </motion.div>
          </div>
        )}
